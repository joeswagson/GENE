...
public abstract class CommandWrapper<T> : Command where T : notnull
{
    ...
    public override object Execute(string[] args)
    {
        ...
        return Routes.TryGetValue(route, out var cmd)
            ? cmd.ExecuteInternal(args.Skip(1).ToArray())
            : throw new ArgumentOutOfRangeException(nameof(args), $"Route {route} was invalid.");
    }
}